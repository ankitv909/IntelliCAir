...
const FullTimeJob: React.FC<FullTimeJobProps> = ({fullTimeJobs, onViewJobClick}) => {
    const user = useAuth()
                                    ...
                                    <div key={job.id} className="card flex flex-col gap-4">
                                        <div className={'flex flex-row gap-4'}>
                                            ...
                                            <div className={'flex flex-col w-full'}>
                                                <div className={'flex flex-col mb-8'}>
                                                    ...
                                                    <h6 className={'text-gray-400'}>{job.city}, {job.state}, USA</h6>
                                                    <h6>{job.hospitalName}</h6>
                                                </div>
                                                <div
                                                    className={'full-time-shade mb-2'}>{formatJobType(job.jobType, ' ')}</div>
                                        ...
                                        <div className="flex flex-row gap-2 justify-end">
                                            ...
                                            <button type="button" onClick={() => onViewJobClick(job)}
                                                    className="apply-blue-button rounded-full font-semibold">
                                                {user.user?.role.toLowerCase() === 'hr' ? 'Bid' : 'Apply'}
                                            </button>
                                            {/* <button onClick={() => onViewJobClick(job)} type="button"
                                                    className="apply-blue-button rounded-full font-semibold">
                                                Apply
                                            </button>*/}
                                        </div>
